...
ALTER TABLE SALES_DATA_FINAL
ADD COLUMN DAY_PASSES INT;
update SALES_DATA_FINAL
SET DAY_PASSES = DATEDIFF('DAY', ORDER_DATE, SHIP_DATE);
...
Select *,                
Case
    When (DAY_PASSES) <= 3 Then 5
    When (DAY_PASSES) > 3 and (DAY_PASSES) <=6  Then 4
    When (DAY_PASSES) > 6 and (DAY_PASSES) <=10  Then 3
    Else 2
End As Rating
From Sales_Data_Final;
